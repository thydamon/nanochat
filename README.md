...
├── README.md
├── dev
│   ├── gen_synthetic_data.py       # 身份认同的示例合成数据
│   ├── generate_logo.html
│   ├── nanochat.png
│   └── repackage_data_reference.py # 预训练数据分片生成
├── nanochat
│   ├── __init__.py                 # 空文件
│   ├── checkpoint_manager.py       # 模型检查点的保存和加载
│   ├── common.py                   # 杂项工具函数
│   ├── core_eval.py                # 评估基础模型的 CORE 分数 (DCLM 论文)
│   ├── dataloader.py               # 可分词的数据分布式加载器
│   ├── dataset.py                  # 预训练数据的下载/读取工具
│   ├── engine.py                   # 高效推理，支持 KV Cache
│   ├── execution.py                # 支持 LLM 执行 Python 代码作为工具
│   ├── gpt.py                      # GPT Transformer 模型实现
│   ├── logo.svg
│   ├── loss_eval.py                # 评估 bits per byte（替代 loss）
│   ├── optim.py                    # AdamW + Muon 优化器，支持单机和分布式
│   ├── report.py                   # 生成 nanochat 实验报告的工具
...
│   └── speedrun.sh                 # 训练约 $100 的 nanochat d20 模型
├── scripts
│   ├── base_eval.py                # 基础模型：评估 CORE 分数、bits per byte、采样
│   ├── base_train.py               # 基础模型：预训练
│   ├── chat_cli.py                 # 对话模型：命令行交互
│   ├── chat_eval.py                # 对话模型：评估任务
│   ├── chat_rl.py                  # 对话模型：强化学习
│   ├── chat_sft.py                 # 对话模型：监督微调
│   ├── chat_web.py                 # 对话模型：Web UI 对话
│   ├── tok_eval.py                 # 分词器：评估压缩率
│   └── tok_train.py                # 分词器：训练
├── tasks
│   ├── arc.py                      # 多选题科学问答
│   ├── common.py                   # TaskMixture | TaskSequence 任务混合/序列
│   ├── customjson.py               # 从任意 jsonl 对话创建任务
│   ├── gsm8k.py                    # 8K 道小学数学题
│   ├── humaneval.py                # Python 编程任务
│   ├── mmlu.py                     # 多选题，涵盖广泛主题
│   ├── smoltalk.py                 # HuggingFace 上的 SmolTalk 数据集集合
│   └── spellingbee.py              # 拼写/计数字母任务
├── tests
│   └── test_engine.py
└── uv.lock
```
